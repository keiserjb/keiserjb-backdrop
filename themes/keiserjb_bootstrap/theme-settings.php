 ...
 * theme-settings.php
 *
 * Theme settings file for keiserjb-bootstrap.
 */

function keiserjb_bootstrap_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL) {
  ...
  $form['bootstrap'] = array(
    '#type' => 'vertical_tabs',
    '#attached' => array(
      'js'  => array(backdrop_get_path('theme', 'keiserjb_bootstrap') . '/js/theme-settings.js'),
    ),
    '#prefix' => '<h2><small>' . t('Bootstrap Settings') . '</small></h2>',
    '#weight' => -10,
  );
  backdrop_add_css(backdrop_get_path('theme', 'keiserjb_bootstrap') . '/css/settings.css');

  // Version and CDN.

  $form['keiserjb_bootstrap_cdn'] = array(
    '#type' => 'fieldset',
  ...
  $form['keiserjb_bootstrap_cdn']['keiserjb_bootstrap_cdn'] = array(
    '#type' => 'select',
    ...
    '#default_value' => theme_get_setting('keiserjb_bootstrap_cdn', $theme_name),
  ...
  $default_theme_details = array(
    ...
    'thumbnail' => base_path() . backdrop_get_path('theme', 'keiserjb_bootstrap') . '/preview.jpg',
  );
  ...
  $form['bootswatch']['keiserjb_bootstrap_bootswatch'] = array(
    '#type' => 'radios',
    '#default_value' => theme_get_setting('keiserjb_bootstrap_bootswatch', $theme_name),
  ...
  if (empty($bootswatch_themes)) {
    $form['bootswatch']['keiserjb_bootstrap_bootswatch']['#prefix'] = '<div class="alert alert-danger messages error"><strong>' . t('ERROR') . ':</strong> ' . t('Unable to reach Bootswatch API. Please ensure the server your website is hosted on is able to initiate HTTP requests.') . '</div>';
  }
  ...
  $form['navbar']['keiserjb_bootstrap_navbar_position'] = array(
    '#type' => 'select',
    '#title' => t('Navbar Position'),
    '#description' => t('Select your Navbar position.'),
    '#default_value' => theme_get_setting('keiserjb_bootstrap_navbar_position', $theme_name),
    '#options' => array(
  ...
  $form['navbar']['keiserjb_bootstrap_navbar_menu_position'] = array(
    '#type' => 'select',
    '#title' => t('Navbar Menu Position'),
    '#description' => t('Select your Navbar Menu position.'),
    '#default_value' => theme_get_setting('keiserjb_bootstrap_navbar_menu_position', $theme_name),
    '#options' => array(
      'navbar-left' => t('Left'),
  ...
  $form['navbar']['keiserjb_bootstrap_navbar_inverse'] = array(
    '#type' => 'checkbox',
    '#title' => t('Inverse navbar style'),
    '#description' => t('Select if you want the inverse navbar style.'),
    '#default_value' => theme_get_setting('keiserjb_bootstrap_navbar_inverse', $theme_name),
  );

  $form['navbar']['keiserjb_bootstrap_navbar_user_menu'] = array(
    '#type' => 'checkbox',
    '#title' => t('Add cog with user-menu'),
    '#description' => t('Select if you want cog style right pulled popup menu.'),
    '#default_value' => theme_get_setting('keiserjb_bootstrap_navbar_user_menu', $theme_name),
  );
  ...
  $form['breadcrumbs']['keiserjb_bootstrap_breadcrumb_home'] = array(
    '#type' => 'checkbox',
    '#title' => t('Show "Home" breadcrumb link'),
    '#default_value' => theme_get_setting('keiserjb_bootstrap_breadcrumb_home', $theme_name),
    '#description' => t('If your site has a module dedicated to handling breadcrumbs already, ensure this setting is enabled.'),
  );
  $form['breadcrumbs']['keiserjb_bootstrap_breadcrumb_title'] = array(
    '#type' => 'checkbox',
    '#title' => t('Show current page title at end'),
    '#default_value' => theme_get_setting('keiserjb_bootstrap_breadcrumb_title', $theme_name),
  ...
  $form['tweaks']['keiserjb_bootstrap_container'] = array(
    '#type' => 'select',
    '#title' => t('Container type'),
    '#default_value' => theme_get_setting('keiserjb_bootstrap_container', $theme_name),
  ...
  $form['tweaks']['keiserjb_bootstrap_datetime'] = array(
    '#type' => 'checkbox',
    '#title' => t('Show "XX time ago".'),
    '#default_value' => theme_get_setting('keiserjb_bootstrap_datetime', $theme_name),
    '#description' => t('If enabled, replace date output for nodes and comments by "XX time ago".'),
  );

}

function keiserjb_bootstrap_bootswatch_template($bootswatch_theme) {
